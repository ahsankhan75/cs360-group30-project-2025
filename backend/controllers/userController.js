...
const signupUser = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Signup successful! Check your email to verify your account." });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
};
...
const signupAdmin = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "Admin signup successful! Check your email to verify your account." });
...
const verifyEmail = async (req, res) => {
  ...
  if (!user)
    return res.status(400).json({ error: 'Verification link invalid or expired.' });
